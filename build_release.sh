...
#----------------------------------------------
#https://github.com/Luo-yi-le/magic.git
project_path=`pwd`
# 构建后的二进制执行文件名
exec_file_name="magic"
# web项目目录
web_folder="${project_path}/magic_web"
# server目录
...
function build() {
    ...
    echo_green "拷贝脚本等资源文件[config.yml、magic.sql、readme.txt、startup.sh、shutdown.sh]"
    cp ${server_folder}/config.yml ${toFolder}
    cp ${server_folder}/magic.sql ${toFolder}
...
function buildLinuxAmd64() {
    build "$1/magic-linux-amd64" "linux" "amd64" $2
}

function buildLinuxArm64() {
    build "$1/magic-linux-arm64" "linux" "arm64" $2
}

function buildWindows() {
    build "$1/magic-windows" "windows" "amd64" $2
}

function buildMac() {
    build "$1/magic-mac" "darwin" "amd64" $2
}
